  ...
  return (
    <section className="bg-white py-16 border-b border-gray-100">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        {/* Header decoration seen in screens */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categories.map((c, i) => (
            <motion.div
              ...
              className="flex items-center justify-between p-5 bg-white border border-gray-200 rounded-xl hover:border-red-500 hover:shadow-sm cursor-pointer transition-all duration-300 group"
            >
              <div className="flex items-center gap-4">
                {/* Icon wrapper */}
                <div className="p-2.5 bg-gray-50 group-hover:bg-red-50 rounded-lg transition-colors">
                  {c.icon}
                </div>
                <span className="font-heading text-base font-bold text-gray-800 group-hover:text-red-600 transition-colors">
                  {c.title}
                </span>
              </div>
              
              {/* Right Arrow */}
              <ArrowRight className="h-5 w-5 text-gray-400 group-hover:text-red-600 group-hover:translate-x-1.5 transition-all" />
            </motion.div>
          ))}
        </div>
